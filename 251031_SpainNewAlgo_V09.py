        ...
        if example_bead_df is None:
            ...
            with tabs[0]:
                ...
                if rows:
                    ...
                    if not df2.empty:
                        fig2 = go.Figure()
                        colors2 = [color_map[s] for s in df2["Status"]]
                        fig2.add_trace(
                            go.Scatter(
                                x=df2["Bead"],
                    ...
                    if not df3.empty:
                        fig3 = go.Figure()
                        colors3 = [color_map[s] for s in df3["Status"]]
                        fig3.add_trace(
                            go.Scatter(
                                x=df3["Bead"],
                                y=df3["Z_Low_Exceed"],
                    ...
                    if not df4.empty:
                        fig4 = go.Figure()
                        colors4 = [color_map[s] for s in df4["Status"]]
                        fig4.add_trace(
                            go.Scatter(
                                x=df4["Bead"],
                                y=df4["Z_High_Exceed"],
                ...
                if not ref_observations or not test_observations:
                    ...
                    if fig_norm_raw is not None:
                        plot_top_signals(
                            ...
                            test_transformed_raw,
                            status_raw,
                            title=f"Raw Signal (Bead {selected_bead}, TEST, colored by global thresholds)",
                ...
                if not ref_observations or not test_observations:
                    ...
                    if fig_norm_sg is not None:
                        plot_top_signals(
                            ...
                            test_transformed_sg,
                            status_sg,
                            title=f"Smoothed Signal (Bead {selected_bead}, TEST, colored by global thresholds)",
                            y_label="Signal Value"
                        )
                ...
                if not ref_observations or not test_observations:
                    ...
                    if fig_norm_lp is not None:
                        plot_top_signals(
                            ...
                            test_transformed_lp,
                            status_lp,
                            title=f"Low-pass Filtered Signal (Bead {selected_bead}, TEST, colored by global thresholds)",
                            y_label="Signal Value"
                        )
                ...
                if not ref_observations or not test_observations:
                    ...
                    if fig_norm_cf is not None:
                        plot_top_signals(
                            ...
                            test_transformed_cf,
                            status_cf,
                            title=f"Curve-fit Signal (Bead {selected_bead}, TEST, colored by global thresholds)",
                            y_label="Signal Value"
                        )
