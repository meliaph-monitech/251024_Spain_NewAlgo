...
def make_obs_id(fname: str, bead_no: int) -> str:
    return f"{fname}__B{bead_no}"
...
def plot_top_signals(ref_transformed, test_transformed, status_map, title, y_label):
    ...
    for obs in test_transformed:
        ...
        x = np.arange(len(y))
        obs_id = obs["csv"]
        status = status_map.get(obs_id, "ok")
        ...
        name = f"{short_label(obs_id)} (TEST, {label})"
        fig.add_trace(
            go.Scatter(
                x=x, y=y, mode="lines",
                name=name,
                line=dict(color=color, width=width),
                legendgroup=short_label(obs_id)
            )
        )
                        ...
                        with st.spinner("Running global summary across both columns (raw only)..."):
                                    ...
                                    for fname in ok_files:
                                        ...
                                        for bead_no, bead_df in beads.items():
                                            if col_name in bead_df.columns:
                                                ...
                                                ref_obs_global.append({
                                                    "csv": make_obs_id(fname, bead_no),
                                                    "data": data,
                                                    "src_csv": fname,
                                                    "bead": bead_no
                                                })
                            ...
                            for bead in bead_options:
                                ...
                                for col_name in cols_local:
                                    ...
                                    for obs_id, status in status_map.items():
                                        if status == "ok":
                                            continue
                            ...
                            for fname in ok_files:
                                ...
                                if selected_bead in beads:
                                    ...
                                    if col_name in bead_df.columns:
                                        data = bead_df[col_name].reset_index(drop=True)
                                        ref_obs.append({
                                            "csv": make_obs_id(fname, selected_bead),
                                            "data": data,
                                            "src_csv": fname,
                                            "bead": selected_bead
                                        })
