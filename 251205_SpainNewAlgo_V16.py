...
if st.session_state.segmented_ok and st.session_state.segmented_test:
    ...
    if not bead_options:
        ...
        if example_bead_df is None:
            ...
            # ------------ Tab 0: Global Summary (All Beads, All Transforms) ------------
            with tabs[0]:
                st.subheader("Global Summary of Suspected NOK (All Beads, All Transforms)")
                ...
                for bead in bead_options:
                    ...
                    ref_obs_bead = []
                    for fname in ok_files:
                        beads_ok = segmented_ok[fname]
                        if bead in beads_ok:
                            bead_df = beads_ok[bead]
                    ...
                    for fname in test_files:
                        beads_test = segmented_test[fname]
                        if bead in beads_test:
                            bead_df = beads_test[bead]
                            if signal_col in bead_df.columns:
                                data = bead_df[signal_col].reset_index(drop=True)
                    ...
                    # Prepare per-file aggregation structure
                    per_file = {}
                    test_csv_names = sorted({obs["csv"] for obs in test_obs_bead})
                    for csv_name in test_csv_names:
                        per_file[csv_name] = {
                            ...
                            "has_low": False,
                            "has_high": False,
                            "Norm_Low_Exceed": None,
                            "Norm_High_Exceed": None,
                            "Z_Low_Exceed": None,
                            "Z_High_Exceed": None,
                        }

                    # List of modes to evaluate in summary
                    summary_modes = [
                        ("raw",      "Raw_NOK",       {}),
                        ("savgol",   "Savitzky_NOK",  {"window": summary_window, "poly": summary_poly}),
                        ("lowpass",  "Lowpass_NOK",   {"cutoff": summary_cutoff, "order": summary_order}),
                        ("poly",     "CurveFit_NOK",  {"deg": summary_deg}),
                    ]

                    for mode_name, nok_key, params in summary_modes:
                        # Transform OK/TEST in this mode
                        ...
                        fig_mode, status_mode, metrics_mode = compute_step_normalization_and_flags(
                            ...
                            title_suffix=f"(Bead #{bead}, {mode_name})"
                        )

                        # If fig_mode is None, something went wrong (e.g., no valid data)
                        if fig_mode is None:
                            continue

                        # Aggregate per-file info for this mode
                        for csv_name in test_csv_names:
                            status = status_mode.get(csv_name, "ok")
                            metrics = metrics_mode.get(csv_name, {})

                            if status in ("low", "high"):
                                per_file[csv_name][nok_key] = 1

                            # Track dominant direction flags
                            if status == "low":
                                per_file[csv_name]["has_low"] = True
                            elif status == "high":
                                per_file[csv_name]["has_high"] = True

                            # Combine metrics (worst across transforms)
                            for metric_key in ["Norm_Low_Exceed", "Norm_High_Exceed", "Z_Low_Exceed", "Z_High_Exceed"]:
                                val = metrics.get(metric_key, np.nan)
                                if np.isnan(val):
                                    continue

                                if metric_key in ["Norm_Low_Exceed", "Z_Low_Exceed"]:
                                    # More negative = worse => take min
                                    current = per_file[csv_name][metric_key]
                                    if current is None or val < current:
                                        per_file[csv_name][metric_key] = val
                                else:
                                    # Higher = worse => take max
                                    current = per_file[csv_name][metric_key]
                                    if current is None or val > current:
                                        per_file[csv_name][metric_key] = val

                    # After all modes, finalize rows for this bead
                    for csv_name, info in per_file.items():
                        # Should we list this row? Only if at least one mode is NOK
                        if (info["Raw_NOK"] == 0 and
                            info["Savitzky_NOK"] == 0 and
                            info["Lowpass_NOK"] == 0 and
                            info["CurveFit_NOK"] == 0):
                            continue
                        ...
                        if info["has_low"]:
                            ...
                            # Should not happen (we would not list row), but just in case
                            status_str = "NOK"

                        # Convert None -> NaN for metrics
                        norm_low_val = info["Norm_Low_Exceed"]
                        norm_high_val = info["Norm_High_Exceed"]
                        z_low_val = info["Z_Low_Exceed"]
                        z_high_val = info["Z_High_Exceed"]

                        rows.append({
                            "CSV_File": csv_name,
                            "Bead": bead,
                            "Status": status_str,
                            "Raw_NOK": info["Raw_NOK"],
                            "Savitzky_NOK": info["Savitzky_NOK"],
                            "Lowpass_NOK": info["Lowpass_NOK"],
                            "CurveFit_NOK": info["CurveFit_NOK"],
                            "Norm_Low_Exceed": np.nan if norm_low_val is None else norm_low_val,
                            "Norm_High_Exceed": np.nan if norm_high_val is None else norm_high_val,
                            "Z_Low_Exceed": np.nan if z_low_val is None else z_low_val,
                            "Z_High_Exceed": np.nan if z_high_val is None else z_high_val,
                        })
                ...
                if rows:
                    ...
                    # - CSV_File name does NOT have "NG" -> Over-detected NOK  -> grey
                    ...
                    if not df1.empty:
                        ...
                        fig1.update_layout(
                            title="Norm_Low_Exceed vs Bead (Combined Across Transforms)",
                    ...
                    if not df2.empty:
                        ...
                        fig2.update_layout(
                            title="Norm_High_Exceed vs Bead (Combined Across Transforms)",
                            xaxis_title="Bead Number",
                    ...
                    if not df3.empty:
                        ...
                        fig3.update_layout(
                            title="Z_Low_Exceed vs Bead (Combined Across Transforms)",
                            xaxis_title="Bead Number",
                    ...
                    if not df4.empty:
                        ...
                        fig4.update_layout(
                            title="Z_High_Exceed vs Bead (Combined Across Transforms)",
                            xaxis_title="Bead Number",
