...
def get_channel_columns(bead_df: pd.DataFrame):
    """
    Avoid hard-coding: interpret "Channel 1" and "Channel 2" as
...
def compute_step_normalization_and_flags(
    ...
    """
    - OK reference center  = per-step median
    - OK reference spread  = per-step std
...
# ============================================================
# SignalTransform identification (worst severity among transforms)
# ============================================================
...
def pick_worst_transform_for_bead(
    ...
    """
    For one bead (and one signal column): evaluate 4 transforms and pick the
    transform with worst severity per TEST csv.
    ...
    for tf_label, tf_mode, tf_params in TRANSFORMS_FOR_SUMMARY:
        ...
        _, status_map, metrics_map = compute_step_normalization_and_flags(
            ...
            z_lower=z_lower,
            z_upper=z_upper,
            title_suffix=f"(Summary • {tf_label})"
        )
...
def plot_top_signals(ref_transformed, test_transformed, status_map, title, y_label):
    ...
    for obs in test_transformed:
        ...
        if status == "low":
            color = "red"; width = 2; label = "LOW"
        elif status == "high":
            color = "orange"; width = 2; label = "HIGH"
        else:
            color = "green"; width = 1; label = "OK-like"
...
if st.session_state.segmented_ok and st.session_state.segmented_test:
    ...
    if not bead_options:
        ...
        if example_bead_df is None:
            ...
            # ------------ Tab 0: Global Summary (All Beads, BOTH CHANNELS) ------------
            with tabs[0]:
                st.subheader("Global Summary of Suspected NOK (All Beads, Both Channels)")

                rows = []

                for bead in bead_options:
                    # Find a representative bead_df (OK) for channel column names
                    ...
                    for ch_idx, ch_col in enumerate(channel_cols, start=1):
                        # Build OK obs for this bead + channel
                        ref_obs_bead = []
                        for fname in ok_files:
                            beads = segmented_ok[fname]
                            if bead in beads:
                        ...
                        # Build TEST obs for this bead + channel
                        test_obs_bead = []
                        ...
                        # Worst transform per CSV for this bead+channel
                ...
                if rows:
                    ...
                    if not df1.empty:
                        ...
                        fig1.add_trace(
                            go.Scatter(
                                ...
                                text=df1["CSV_File"] + " | " + df1["Channel"],
                                hovertemplate="ID: %{text}<br>Bead: %{x}<br>Norm_Low_Exceed: %{y}<extra></extra>"
                            )
                        )
                        fig1.update_layout(
                            title="Norm_Low_Exceed vs Bead (Both Channels)",
                    ...
                    if not df2.empty:
                        ...
                        fig2.add_trace(
                            go.Scatter(
                                ...
                                text=df2["CSV_File"] + " | " + df2["Channel"],
                                hovertemplate="ID: %{text}<br>Bead: %{x}<br>Norm_High_Exceed: %{y}<extra></extra>"
                            )
                        )
                        fig2.update_layout(
                            title="Norm_High_Exceed vs Bead (Both Channels)",
                            xaxis_title="Bead Number",
                    ...
                    if not df3.empty:
                        ...
                        fig3.add_trace(
                            go.Scatter(
                                ...
                                text=df3["CSV_File"] + " | " + df3["Channel"],
                                hovertemplate="ID: %{text}<br>Bead: %{x}<br>Z_Low_Exceed: %{y}<extra></extra>"
                            )
                        )
                        fig3.update_layout(
                            title="Z_Low_Exceed vs Bead (Both Channels)",
                            xaxis_title="Bead Number",
                    ...
                    if not df4.empty:
                        ...
                        fig4.add_trace(
                            go.Scatter(
                                ...
                                text=df4["CSV_File"] + " | " + df4["Channel"],
                                hovertemplate="ID: %{text}<br>Bead: %{x}<br>Z_High_Exceed: %{y}<extra></extra>"
                            )
                        )
                        fig4.update_layout(
                            title="Z_High_Exceed vs Bead (Both Channels)",
                            xaxis_title="Bead Number",
                    ...
                    st.info("No suspected NOK beads found with current thresholds (both channels).")

            # ------------ Build per-bead observations for detailed tabs (selected signal_col) ------------
            ref_observations = []
            for fname in ok_files:
                beads = segmented_ok[fname]
                if selected_bead in beads:
