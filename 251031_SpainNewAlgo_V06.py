...
def compute_step_normalization_and_flags(
    ...
    # --- Plot TEST signals and compute status ---
    for obs in test_obs:
        y = np.asarray(obs["transformed"])
        x = np.arange(len(y))
        _, step_y = aggregate_for_step(x, y, step_interval)
        ...
        low_flag = np.any(norm_vals < norm_lower) or np.any(z_vals < -z_lower)
        # High deviation (too high) condition
        high_flag = np.any(norm_vals > norm_upper) or np.any(z_vals > z_upper)
        ...
        if example_bead_df is None:
            ...
            if len(ref_observations) == 0:
                ...
                with tabs[0]:
                    ...
                    fig_norm_raw, status_raw = compute_step_normalization_and_flags(
                        ref_transformed_raw,
                ...
                with tabs[1]:
                    ...
                    fig_norm_sg, status_sg = compute_step_normalization_and_flags(
                        ref_transformed_sg,
                        test_transformed_sg,
                        step_interval=global_step_interval,
                        norm_lower=global_norm_lower,
                ...
                with tabs[2]:
                    ...
                    fig_norm_lp, status_lp = compute_step_normalization_and_flags(
                        ref_transformed_lp,
                        test_transformed_lp,
                        step_interval=global_step_interval,
                        norm_lower=global_norm_lower,
                ...
                with tabs[3]:
                    ...
                    fig_norm_cf, status_cf = compute_step_normalization_and_flags(
                        ref_transformed_cf,
                        test_transformed_cf,
                        step_interval=global_step_interval,
                        norm_lower=global_norm_lower,
