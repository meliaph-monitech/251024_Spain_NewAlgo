...
if uploaded_ok_zip:
    ...
    # Perform segmentation for OK set (keep your original pattern)
...
if st.session_state.segmented_ok and st.session_state.segmented_test:
    ...
    if not bead_options:
        ...
        if example_bead_df is None:
            ...
            # UI display remains controlled by user-selected signal column
            ...
            # ------------ Tab 0: Global Summary (All Beads, BOTH CHANNELS, LONG-FORM per-transform table) ------------
            with tabs[0]:
                st.subheader("Global Summary of Suspected NOK (All Beads, Both Channels, Per Transform)")

                rows = []

                with st.spinner("Running global summary across both channels and all transforms..."):
                    for bead in bead_options:
                        ...
                        for ch_idx, ch_col in enumerate(channel_cols):  # 0, 1
                            # build raw observations for this bead/channel
                            ref_obs_bead = []
                            for fname in ok_files:
                                beads = segmented_ok[fname]
                                if bead in beads:
                            ...
                            # IMPORTANT CHANGE:
                            # Instead of "worst transform", evaluate each transform separately and append rows per transform.
                            for tf_label, tf_mode, tf_params in TRANSFORMS_FOR_SUMMARY:
                                ref_t = compute_transformed_signals(ref_obs_bead, mode=tf_mode, **tf_params)
                                test_t = compute_transformed_signals(test_obs_bead, mode=tf_mode, **tf_params)

                                # We don't need the figure in summary, but keep call unchanged.
                                _, status_map, metrics_map = compute_step_normalization_and_flags(
                                    ref_t,
                                    test_t,
                                ...
                                for csv_name, status in status_map.items():
                                    ...
                                    rows.append({
                                        ...
                                        "SignalTransform": tf_label,  # per-transform (long-form)
                ...
                if rows:
                    ...
                    st.info("No suspected NOK beads found with current thresholds (both channels, all transforms).")
            ...
            with tabs[2]:
                ...
                if not ref_observations or not test_observations:
                    ...
                    fig_norm_sg, status_sg, _ = compute_step_normalization_and_flags(
                        ref_transformed_sg, test_transformed_sg,
                        step_interval=global_step_interval,
                        norm_lower=global_norm_lower, norm_upper=global_norm_upper,
                        z_lower=global_z_lower, z_upper=global_z_upper,
                    ...
                    if fig_norm_sg is not None:
                        plot_top_signals(
                            ref_transformed_sg, test_transformed_sg, status_sg,
            ...
            with tabs[3]:
                ...
                if not ref_observations or not test_observations:
                    ...
                    fig_norm_lp, status_lp, _ = compute_step_normalization_and_flags(
                        ref_transformed_lp, test_transformed_lp,
                        step_interval=global_step_interval,
                        norm_lower=global_norm_lower, norm_upper=global_norm_upper,
                        z_lower=global_z_lower, z_upper=global_z_upper,
                    ...
                    if fig_norm_lp is not None:
                        plot_top_signals(
                            ref_transformed_lp, test_transformed_lp, status_lp,
            ...
            with tabs[4]:
                ...
                if not ref_observations or not test_observations:
                    ...
                    fig_norm_cf, status_cf, _ = compute_step_normalization_and_flags(
                        ref_transformed_cf, test_transformed_cf,
                        step_interval=global_step_interval,
                        norm_lower=global_norm_lower, norm_upper=global_norm_upper,
                        z_lower=global_z_lower, z_upper=global_z_upper,
                    ...
                    if fig_norm_cf is not None:
                        plot_top_signals(
                            ref_transformed_cf, test_transformed_cf, status_cf,
