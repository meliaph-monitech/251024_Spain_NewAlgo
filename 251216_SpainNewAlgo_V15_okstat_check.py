...
#         + (NEW) OK step meta preview table
# ============================================================
def compute_step_normalization_and_flags(
    ...
    z_upper,
    title_suffix,
    preview_steps=5,          # NEW: number of step bins to preview
    preview_mode="first"      # "first" (default) or "top_abs"
):
    """
    ...
    Returns:
      fig, status_map, metrics_map, ok_preview_df
    """
    ...
    # ---- NEW: Build OK step meta preview table (top 5 steps per CSV) ----
    preview_rows = []
        ...
        if example_bead_df is None:
            ...
            with tabs[0]:
                ...
                for bead in bead_options:
                    ...
                    _, status_bead, metrics_bead, _ok_prev = compute_step_normalization_and_flags(
            ...
            with tabs[2]:
                ...
                if not ref_observations or not test_observations:
                    ...
                    ref_transformed_sg = compute_transformed_signals(
                        ref_observations, mode="savgol", window=window, poly=poly
                    )
                    test_transformed_sg = compute_transformed_signals(
                        test_observations, mode="savgol", window=window, poly=poly
                    )
            ...
            with tabs[3]:
                ...
                if not ref_observations or not test_observations:
                    ...
                    ref_transformed_lp = compute_transformed_signals(
                        ref_observations, mode="lowpass", cutoff=cutoff, order=order
                    )
                    test_transformed_lp = compute_transformed_signals(
                        test_observations, mode="lowpass", cutoff=cutoff, order=order
                    )
            ...
            with tabs[4]:
                ...
                if not ref_observations or not test_observations:
                    ...
                    ref_transformed_cf = compute_transformed_signals(
                        ref_observations, mode="poly", deg=deg
                    )
                    test_transformed_cf = compute_transformed_signals(
                        test_observations, mode="poly", deg=deg
                    )
