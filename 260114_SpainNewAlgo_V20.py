...
def get_channel_columns(bead_df: pd.DataFrame):
    """
    No hard-coding: Channel 0 and Channel 1 are interpreted as
...
if st.session_state.segmented_ok and st.session_state.segmented_test:
    ...
    if not bead_options:
        ...
        # Find an example bead df to infer channel columns (first/second columns)
        ...
        if example_bead_df is None:
            ...
            if len(channel_cols) < 2:
                st.error("Bead dataframe has fewer than 2 columns, cannot create Channel 0/1 tabs.")
            else:
                ch0_col, ch1_col = channel_cols[0], channel_cols[1]

                st.markdown("### Analysis (RAW ONLY)")
                st.markdown(
                    f"- OK reference files: {len(ok_files)}  \n"
                    f"- TEST files: {len(test_files)}  \n"
                    f"- Selected Bead: **#{selected_bead}**  \n"
                    f"- Channel 0 Column: **{ch0_col}**  \n"
                    f"- Channel 1 Column: **{ch1_col}**"
                )
                ...
                with tabs[0]:
                    ...
                    with st.spinner("Running global summary across both channels (raw only)..."):
                        for bead in bead_options:
                            ...
                            channel_cols_local = get_channel_columns(bead_df_for_cols)
                            if len(channel_cols_local) < 2:
                                continue

                            for ch_idx, ch_col in enumerate(channel_cols_local):  # 0, 1
                                ref_obs_bead = []
                                for fname in ok_files:
                                    beads = segmented_ok[fname]
                                    if bead in beads:
                ...
                # ------------ Helper: build observations for a given channel column ------------
                def build_observations_for_channel(col_name: str):
                    ref_obs = []
                    for fname in ok_files:
                        beads = segmented_ok[fname]
                        if selected_bead in beads:
                ...
                # ------------ Tab 1: Channel 0 ------------
                with tabs[1]:
                    st.subheader("Channel 0 (Top) + Per-step Normalization (Bottom)")
                    ref_observations, test_observations = build_observations_for_channel(ch0_col)
                    ...
                    if not ref_observations or not test_observations:
                        ...
                        fig_norm_raw, status_raw, _ = compute_step_normalization_and_flags(
                            ...
                            title_suffix=f"• Channel 0 ({ch0_col}) • Bead #{selected_bead}"
                        )
                        ...
                        if fig_norm_raw is not None:
                            plot_top_signals(
                                ...
                                title=(
                                    f"Channel 0 ({ch0_col}) • Bead #{selected_bead} • "
                ...
                # ------------ Tab 2: Channel 1 ------------
                with tabs[2]:
                    st.subheader("Channel 1 (Top) + Per-step Normalization (Bottom)")
                    ref_observations, test_observations = build_observations_for_channel(ch1_col)
                    ...
                    if not ref_observations or not test_observations:
                        ...
                        fig_norm_raw, status_raw, _ = compute_step_normalization_and_flags(
                            ...
                            norm_lower=global_norm_lower, norm_upper=global_norm_upper,
                            z_lower=global_z_lower, z_upper=global_z_upper,
                            title_suffix=f"• Channel 1 ({ch1_col}) • Bead #{selected_bead}"
                        )
                        ...
                        if fig_norm_raw is not None:
                            plot_top_signals(
                                ref_transformed_raw, test_transformed_raw, status_raw,
                                title=(
                                    f"Channel 1 ({ch1_col}) • Bead #{selected_bead} • "
                                    f"Recipe: Norm[{global_norm_lower},{global_norm_upper}] "
                                    f"Z-score[-{global_z_lower},{global_z_upper}] "
                                    f"Step[{global_step_interval}]"
                                ),
