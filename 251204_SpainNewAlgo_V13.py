...
def compute_step_normalization_and_flags(
    ...
    # TEST: pre-aggregate as well so we can compute a global min_steps
    test_step_arrays = []
    test_step_meta = []
    ...
    if len(test_step_arrays) == 0:
        # We can still show only OK reference curves if desired,
        # but there are no TEST signals to flag.
        st.warning("No valid TEST step data for normalization.")
        # Build a figure with only OK reference, or just return empty.
        # Here we return empty maps.
        return None, {}, {}
    ...
    for meta in test_step_meta:
        ...
        high_flag = mask_norm_high.any() or mask_z_high.any() or False
