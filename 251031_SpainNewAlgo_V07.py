...
# ============================================================
# Build summary table from status + metrics (per tab)
# ============================================================
def build_summary_table(status_map, metrics_map, bead_number):
    rows = []
    for csv_name, status in status_map.items():
        if status == "ok":
            continue  # only suspected NOK (low or high)
        m = metrics_map.get(csv_name, {})
        rows.append({
            "CSV": short_label(csv_name),
            "Bead": bead_number,
        ...
        if example_bead_df is None:
            ...
            if len(ref_observations) == 0:
                ...
                with tabs[0]:
                    ...
                    fig_norm_raw, status_raw, metrics_raw = compute_step_normalization_and_flags(
                        ref_transformed_raw,
                    ...
                    if fig_norm_raw is not None:
                        ...
                        # Summary table for suspected NOK (Raw)
                        df_raw = build_summary_table(status_raw, metrics_raw, selected_bead)
                        if df_raw is not None:
                            st.markdown("#### Suspected NOK Summary (Raw Signal)")
                ...
                with tabs[1]:
                    ...
                    fig_norm_sg, status_sg, metrics_sg = compute_step_normalization_and_flags(
                        ref_transformed_sg,
                        test_transformed_sg,
                        step_interval=global_step_interval,
                        norm_lower=global_norm_lower,
                    ...
                    if fig_norm_sg is not None:
                        ...
                        df_sg = build_summary_table(status_sg, metrics_sg, selected_bead)
                        if df_sg is not None:
                            st.markdown("#### Suspected NOK Summary (Smoothed)")
                ...
                with tabs[2]:
                    ...
                    fig_norm_lp, status_lp, metrics_lp = compute_step_normalization_and_flags(
                        ref_transformed_lp,
                        test_transformed_lp,
                        step_interval=global_step_interval,
                        norm_lower=global_norm_lower,
                    ...
                    if fig_norm_lp is not None:
                        ...
                        df_lp = build_summary_table(status_lp, metrics_lp, selected_bead)
                        if df_lp is not None:
                            st.markdown("#### Suspected NOK Summary (Low-pass)")
                ...
                with tabs[3]:
                    ...
                    fig_norm_cf, status_cf, metrics_cf = compute_step_normalization_and_flags(
                        ref_transformed_cf,
                        test_transformed_cf,
                        step_interval=global_step_interval,
                        norm_lower=global_norm_lower,
                    ...
                    if fig_norm_cf is not None:
                        ...
                        df_cf = build_summary_table(status_cf, metrics_cf, selected_bead)
                        if df_cf is not None:
                            st.markdown("#### Suspected NOK Summary (Curve Fit)")
