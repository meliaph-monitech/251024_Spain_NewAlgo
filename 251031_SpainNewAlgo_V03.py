    ...
    with zipfile.ZipFile(uploaded_ok_zip, 'r') as zip_ref:
        ...
        if not csv_names_ok:
            ...
            st.session_state.seg_thresh = st.sidebar.number_input(
                "Segmentation Threshold (OK & Test will share this)",
                value=1.0
            )
...
def compute_step_normalization_and_flags(
    ...
    Returns: (fig, flagged_map)
        ...
        - flagged_map: {csv_name: bool} True if flagged
    """
    if len(ref_obs) == 0:
    ...
    # Plot TEST signals using OK-based normalization
    for obs in test_obs:
        y = np.asarray(obs["transformed"])
        x = np.arange(len(y))
        _, step_y = aggregate_for_step(x, y, step_interval)
        ...
        # Flag if any step violates thresholds
...
# ============================================================
# Helper: Plot Top (Transformed TEST Signals with flags)
# ============================================================
def plot_top_signals(transformed_test_obs, flagged_map, title, y_label):
    fig = go.Figure()
    for obs in transformed_test_obs:
        y = obs["transformed"]
        x = np.arange(len(y))
        csv_name = obs["csv"]
        ...
        color = "red" if flagged else "green"
        width = 2 if flagged else 1
        name = f"{csv_name} ({'FLAGGED' if flagged else 'OK-like'})"
        fig.add_trace(
            go.Scatter(
                x=x,
                y=y,
...
if st.session_state.segmented_ok and st.session_state.segmented_test:
    st.sidebar.header("Step 3: Bead & Signal for Analysis")
        ...
        if example_bead_df is None:
            ...
            if len(ref_observations) == 0:
                ...
                with tabs[0]:
                    ...
                    norm_thresh_raw = st.number_input(
                        "Lower Threshold for 0–1 Normalization (flag if below)",
                        min_value=-1.0,
                        max_value=1.0,
                        value=0.0,
                        step=0.05
                    )
                    z_thresh_raw = st.number_input(
                        "Z-score Threshold (flag if below -T)",
                        min_value=0.5,
                        max_value=6.0,
                        value=3.0,
                        step=0.5
                    ...
                    fig_norm_raw, flagged_raw = compute_step_normalization_and_flags(
                        ...
                        norm_thresh=norm_thresh_raw,
                        z_thresh=z_thresh_raw,
                    ...
                    if fig_norm_raw is not None:
                        plot_top_signals(
                            test_transformed_raw,
                            flagged_raw,
                            title="Raw Signal (TEST, colored by threshold flags)",
                ...
                with tabs[1]:
                    ...
                    norm_thresh_sg = st.number_input(
                        "Lower Threshold for 0–1 Normalization (Smoothed, flag if below)",
                        min_value=-1.0,
                        max_value=1.0,
                        value=0.0,
                        step=0.05
                    )
                    z_thresh_sg = st.number_input(
                        "Z-score Threshold (Smoothed, flag if below -T)",
                        min_value=0.5,
                        max_value=6.0,
                        value=3.0,
                        step=0.5
                    ...
                    fig_norm_sg, flagged_sg = compute_step_normalization_and_flags(
                        ...
                        norm_thresh=norm_thresh_sg,
                        z_thresh=z_thresh_sg,
                    ...
                    if fig_norm_sg is not None:
                        plot_top_signals(
                            test_transformed_sg,
                            flagged_sg,
                            title="Smoothed Signal (TEST, colored by threshold flags)",
                            y_label="Signal Value"
                        )
                ...
                with tabs[2]:
                    ...
                    norm_thresh_lp = st.number_input(
                        "Lower Threshold for 0–1 Normalization (Low-pass, flag if below)",
                        min_value=-1.0,
                        max_value=1.0,
                        value=0.0,
                        step=0.05
                    )
                    z_thresh_lp = st.number_input(
                        "Z-score Threshold (Low-pass, flag if below -T)",
                        min_value=0.5,
                        max_value=6.0,
                        value=3.0,
                        step=0.5
                    ...
                    fig_norm_lp, flagged_lp = compute_step_normalization_and_flags(
                        ...
                        norm_thresh=norm_thresh_lp,
                        z_thresh=z_thresh_lp,
                    ...
                    if fig_norm_lp is not None:
                        plot_top_signals(
                            test_transformed_lp,
                            flagged_lp,
                            title="Low-pass Filtered Signal (TEST, colored by threshold flags)",
                            y_label="Signal Value"
                        )
                ...
                with tabs[3]:
                    ...
                    norm_thresh_cf = st.number_input(
                        "Lower Threshold for 0–1 Normalization (Curve Fit, flag if below)",
                        min_value=-1.0,
                        max_value=1.0,
                        value=0.0,
                        step=0.05
                    )
                    z_thresh_cf = st.number_input(
                        "Z-score Threshold (Curve Fit, flag if below -T)",
                        min_value=0.5,
                        max_value=6.0,
                        value=3.0,
                        step=0.5
                    ...
                    fig_norm_cf, flagged_cf = compute_step_normalization_and_flags(
                        ...
                        norm_thresh=norm_thresh_cf,
                        z_thresh=z_thresh_cf,
                    ...
                    if fig_norm_cf is not None:
                        plot_top_signals(
                            test_transformed_cf,
                            flagged_cf,
                            title="Curve-fit Signal (TEST, colored by threshold flags)",
                            y_label="Signal Value"
                        )
