...
def compute_step_normalization_and_flags(
    ...
    Robust version (Option B):
    - OK reference center = per-step median
    - OK reference spread = per-step MAD (scaled to sigma_robust)
    - Norm band = median ± k * sigma_robust (k fixed here)
    - Z = (TEST - median) / sigma_robust
    """
    ...
    # TEST: pre-aggregate as well so we can compute a common min_steps
    test_step_arrays = []
    test_step_meta = []
    ...
    # ---- Robust OK statistics per step ----
    # Center: median
    median_ok = np.median(ok_matrix, axis=0)

    # Spread: MAD (median absolute deviation) -> sigma_robust
    mad = np.median(np.abs(ok_matrix - median_ok), axis=0)
    mad[mad < 1e-12] = 1e-12
    sigma_robust = 1.4826 * mad  # scale factor to be comparable to std

    # Robust band for normalization: median ± k * sigma_robust
    band_k = 3.0  # you can tune this later or expose it as a parameter
    low_band = median_ok - band_k * sigma_robust
    high_band = median_ok + band_k * sigma_robust

    # Protect denominator
    denom = high_band - low_band
    ...
    fig = make_subplots(
        rows=1,
        cols=2,
        subplot_titles=("0–1 Normalization (robust OK-based)", "Robust Z-score per Step")
    )
    ...
    for meta in ok_step_meta:
        ...
        norm_ok = (step_y_ok - low_band) / denom
        z_ok = (step_y_ok - median_ok) / sigma_robust
    ...
    for meta in test_step_meta:
        ...
        norm_vals = (step_y - low_band) / denom
        z_vals = (step_y - median_ok) / sigma_robust
        ...
        # Left: robust 0–1 normalization (TEST)
        fig.add_trace(
            go.Scatter(
                x=step_indices,
                y=norm_vals,
    ...
    fig.update_yaxes(title_text="Normalized Value (robust)", row=1, col=1)
    fig.update_xaxes(title_text="Step Index", row=1, col=2)
    fig.update_yaxes(title_text="Robust Z-score", row=1, col=2)
